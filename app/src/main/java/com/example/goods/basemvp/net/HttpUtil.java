...
import com.example.goods.basemvp.model.BaseInone;
import com.example.goods.common.Constants;
import com.example.goods.basemvp.model.InitialApi;
import com.example.goods.utils.DigestUtils;
import com.example.goods.utils.SpUtils;
import com.example.goods.utils.SystemUtils;
...
public class HttpUtil {
    ...
    private static String signUrl = "cdwan.cn"; //需要签名的域名  192.168.43.220   192.168.43.164
    ...
    public InitialApi getInitialApi() {
        if (initialApi == null) {
            initialApi = getRetrofit(Constants.Base_WanUrl).create(InitialApi.class);
        }
        return initialApi;
    }
    private BaseInone baseInone;

    public BaseInone getOne() {
        if (baseInone == null) {
            baseInone = getRetrofit(Constants.Base_One).create(BaseInone.class);
        }
        return baseInone;
    }
    //TPApi end
